...
def inference(args):
    ...
    if isinstance(ref_dict, dict) and ref_dict:
        for key, chars in ref_dict.items():
            print(f"  Characters loaded for key '{key}': {len(chars)}")
    else:
        print("  No reference characters loaded.")
    ...
    # --- MODIFIED: Check character count from ref_dict values ---
    total_ref_chars_loaded = sum(len(chars) for chars in ref_dict.values()) if isinstance(ref_dict, dict) else 0
    if not gen_chars or total_ref_chars_loaded == 0: # Check filtered gen_chars
        print(f"Error: No characters left to generate ({len(gen_chars)}) or no reference characters loaded ({total_ref_chars_loaded}). Skipping inference.")
        return args.output_dir

    # --- ADDED: Log GPU memory before calling infer_DM ---
    try:
        print("--- Checking GPU Memory Usage (before infer_DM) ---")
        ...
        print("nvidia-smi command not found. Skipping GPU memory log.")
